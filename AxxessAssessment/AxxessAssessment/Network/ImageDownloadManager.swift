...
final class ImageDownloadManager {
    ...
    lazy var imageDownloadQueue: OperationQueue = {
        ...
        queue.name = "com.axxess.imageDownloadQueue"
    ...
    func downloadImage(_ urlString: String, indexPath: IndexPath?, handler: @escaping ImageDownloadHandler) {
        ...
        if let cachedImage = imageCache.object(forKey: url.absoluteString as NSString) {
           ...
           self.completionHandler?(cachedImage, url, indexPath, nil)
            ...
            if let operations = (imageDownloadQueue.operations as? [AxxessOperation])?.filter({$0.imageUrl.absoluteString == url.absoluteString && $0.isFinished == false && $0.isExecuting == true }), let operation = operations.first {
                print("Increase the priority for \(url)")
                operation.queuePriority = .veryHigh
            } else {
                /* create a new task to download the image.  */
                print("Create a new task for \(url)")
                let operation = AxxessOperation(url: url, indexPath: indexPath)
                if indexPath == nil {
